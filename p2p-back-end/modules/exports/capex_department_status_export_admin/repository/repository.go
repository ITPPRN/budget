...
func (r *repository) GetCapexDeptStatus(ctx context.Context, filter map[string]interface{}) ([]models.CapexDeptStatusDTO, error) {
	...
	btx := r.db.Table("capex_budget_fact_entities").Select("department, SUM(year_total) as total")
	...
	atx := r.db.Table("capex_actual_fact_entities").Select("department, SUM(year_total) as total")
