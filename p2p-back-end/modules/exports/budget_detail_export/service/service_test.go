...
type MockBudgetExportRepository struct {
	mock.Mock
}

func (m *MockBudgetExportRepository) GetBudgetExportDetails(ctx context.Context, user *models.UserInfo, filter map[string]interface{}) ([]models.BudgetExportDTO, error) {
...
// Compile-time check
var _ repository.BudgetExportRepository = (*MockBudgetExportRepository)(nil)
...
func TestNewService(t *testing.T) {
	repo := new(MockBudgetExportRepository)
	svc := NewService(repo)
	assert.NotNil(t, svc)
}

func TestExportBudgetDetailExcel_Success_WithData(t *testing.T) {
	repo := new(MockBudgetExportRepository)
	svc := NewService(repo)
	...
	repo.On("GetBudgetExportDetails", mock.Anything, user, filter).
		Return(rows, nil)

	data, filename, err := svc.ExportBudgetDetailExcel(context.Background(), user, filter)

	assert.NoError(t, err)
	assert.NotEmpty(t, data)
	assert.Contains(t, filename, "Budget_Detail_")
...
func TestExportBudgetDetailExcel_Success_EmptyData(t *testing.T) {
	repo := new(MockBudgetExportRepository)
	svc := NewService(repo)
...
func TestExportBudgetDetailExcel_RepoError(t *testing.T) {
	repo := new(MockBudgetExportRepository)
	svc := NewService(repo)
...
func TestExportBudgetDetailExcel_FilenameFormat(t *testing.T) {
	repo := new(MockBudgetExportRepository)
	svc := NewService(repo)
	...
	_, filename, err := svc.ExportBudgetDetailExcel(context.Background(), user, filter)

	assert.NoError(t, err)
	assert.Regexp(t, `^Budget_Detail_\d{14}\.xlsx$`, filename)

	repo.AssertExpectations(t)
}

func TestExportBudgetDetailExcel_SheetName(t *testing.T) {
	repo := new(MockBudgetExportRepository)
	svc := NewService(repo)
	...
	data, _, err := svc.ExportBudgetDetailExcel(context.Background(), user, filter)
